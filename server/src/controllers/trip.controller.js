 ...
 * Requires a message explaining the deletion
 */
export const deleteTripByAdmin = async (req, res, next) => {
  try {
    ...
    const { adminDeleteTripSchema } =
      await import("#validations/trip.validation.js");
    const { formatValidationError } = await import("#utils/format.js");

    const validationResult = adminDeleteTripSchema.safeParse(req.body);
    if (!validationResult.success) {
      return res.status(400).json({
        error: "Validation failed",
        details: formatValidationError(validationResult.error),
    ...
    const { message, reportId } = validationResult.data;
    ...
    const Notification = (await import("#models/notification.model.js"))
      .default;
    const AdminActionLog = (await import("#models/adminActionLog.model.js"))
      .default;
    ...
    await AdminActionLog.create({
      admin: adminId,
      actionType: "trip_deleted_from_report",
      targetType: "trip",
      targetId: tripId,
      message: message,
      metadata: {
    ...
    // Get all reports for this trip
    const reports = await Report.find({ trip: tripId, status: "pending" })
    ...
    // Collect all users to notify (owner, members, reporters)
    const notificationsToCreate = [];

    // Notify trip owner
    notificationsToCreate.push({
      user: trip.owner._id,
      type: "trip_deleted_from_report",
      title: "Trip Deleted by Admin",
      message: `Your trip "${trip.title}" has been deleted by an administrator. Reason: ${message}`,
    });

    // Notify all members (excluding owner)
      ...
      trip.members
        ...
        .forEach((memberId) => {
          notificationsToCreate.push({
            ...
            type: "trip_deleted_from_report",
            title: "Trip Deleted by Admin",
            message: `The trip "${trip.title}" has been deleted by an administrator. Reason: ${message}`,
          });
        });
    }

    // Notify all reporters of this trip (excluding duplicates)
    ...
    reports.forEach((report) => {
      if (
        report.reporter &&
        !notifiedUsers.has(report.reporter._id.toString())
      ) {
        notificationsToCreate.push({
          user: report.reporter._id,
          type: "trip_deleted_from_report",
          title: "Reported Trip Deleted",
          message: `The trip "${trip.title}" that you reported has been deleted by an administrator. Admin message: ${message}`,
        });
        notifiedUsers.add(report.reporter._id.toString());
      }
    });
